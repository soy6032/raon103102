...
// exports.getTemplate = async (req, res, next) => {
//     resObj = obj_init(resObj);
//     const query = `SELECT t.idx, t.name, t.contents, t.provider, IF(t.price = '0', '무료', t.price) as price, ti.img_path FROM makeredu.template as t join makeredu.template_img as ti on t.idx = ti.t_group WHERE ti.order = 1 ORDER BY ti.idx ASC`;
//     const result = await db.conn(query);
//     if(!result) {
//         resObj.msg = "템플릿 가져오기 실패하였습니다.";
//         resObj.state = false;
...
//       req.body.pdx
//     ];
//     const check_query = `select pm.* from makeredu.users u join makeredu.project_members pm on u.idx = pm.u_idx where u.email = ? and pm.p_idx = ?`;
//     const check_result = await db.conn(check_query, params);
//     if(check_result.length > 0) {
//         resObj.items = true;
//         resObj.state = true;
...
//         req.body.email
//     ];
//     const creator_query = "select u.idx, (select u_idx from makeredu.project where idx = ?) as creator from makeredu.users u where u.email = ?";
//     const creator_result = await db.conn(creator_query, params2);
//     //console.log(creator_result);
//     if(creator_result.length === 0) {
//         resObj.state = false;
...
//     const pdx = req.body.pdx;
//     const query = "SELECT u.email, pm.nick_name as nick from makeredu.project_members pm join makeredu.users u on pm.u_idx = u.idx where pm.p_idx = ?";
//     const result = await db.conn(query, [pdx]);
...
//     ];
//     const query = `select u.email, p.*, pm.creator 
//                     from makeredu.users u 
//                     join makeredu.project_members pm on u.idx = pm.u_idx
//                     join ( select p2.idx, p2.t_idx, p2.u_idx, p2.name, u2.email as creator_email, p2.url, p2.contents, p2.end, ti.img_path , DATE_FORMAT(p2.print_date, '%Y-%m-%d') as print_date, DATE_FORMAT(p2.close_date, '%Y-%m-%d') as close_date, p2.regdate
//                             from makeredu.users as u2
//                             join makeredu.project p2 on u2.idx  = p2.u_idx
//                             join makeredu.template_img ti on p2.t_idx = ti.t_group 
//                             where ti.order = 1) as p on pm.p_idx = p.idx
//                     where u.email = ? and p.end = ? order by p.regdate desc`;
//     const result = await db.conn(query, params);
//     if(!result) {
...
//         req.body.select
//     ];
//     const find_query = "select * from makeredu.images where `p_url` = ? and `section` = ? and `select` = ?";
//     const find_result = await db.conn(find_query, params);
//     let query = "";
//     let result = "";
//     //console.log(find_result.length);
...
//         });
//         params.unshift(req.body.email, path);
//         query = "update makeredu.images set `email` = ?, `path` = ? where `p_url` = ? and `section` = ? and `select` = ?"; 
        
//     } else {
//         params.push(req.body.email, path);
//         query = "insert into makeredu.images(`p_url`, `section`, `select`, `email`, `path`) values(?, ?, ?, ?, ?)";
//     }
...
//     } else {
//         get_params.push(req.body.url, req.body.section, req.body.select);
//         get_query = "select * from makeredu.images where `p_url` = ? and `section` = ? and `select` = ?";
//     }
//     const get_result = await db.conn(get_query, get_params);
//     if(!get_result) {
//         resObj.state = false;
...
//         req.body.idx
//     ]
//     const query = "SELECT pm.creator from makeredu.project p join makeredu.project_members pm on p.idx = pm.p_idx where p.url = ? and pm.u_idx = ?";
//     const result = await db.conn(query, params);
//     //console.log(result);
//     if(result[0].creator !== 1) {
//         resObj.state = false;
...
//     const query = `update makeredu.users set password = password(?) where email = ?`;
//     const result = await db.tran(query, params);
//     if(result.changedRows !== 1) {
//         resObj.state = false;
//         resObj.msg = "잠시 후 다시 시도하세요.";
